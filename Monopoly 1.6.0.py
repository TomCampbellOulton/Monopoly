...
def displayMainBoard(allHousesList, properties=[], colours=["Purple", "Red", "Orange", "Blue"], playerCoords={"Purple":[1336, 893], "Red":[1366, 893], "Orange":[1336, 923], "Blue":[1366, 923]}, buttons=[], playersData=[]):
    # Main Menu Button
    ...
    playersCoords = displayAllPlayers(colours, playerCoords, playersData)
...
def displayAllPlayers(colours, coordinates, players):
    playersCoords = {}
...
def checkTile(tile, player, playersData, properties, reallyPlaying, playersName, allHousesList):
    ...
    # Otherwise the tile is a property ;)
        ...
        print(f"Tile is owned by {properties[tile-1][2]} and player is {playersName}")
        ...
        # If the player owns the property
            ...
            if ownFullSet == True:
                ...
                properties, playersData, allHousesList = validateBuildingRequests(properties, player, playersData, allHousesList, playersName)
        ...
        elif owner == playersName:
            ownFullSet = checkIfPlayerOwnsTheFullSet(properties, tile)
            if ownFullSet:
                # Checks if they want to build
                properties, playersData, allHousesList = validateBuildingRequests(properties, player, playersData, allHousesList, playersName)
...
def checkIfTheUserWantsTheProperty(price, tile, reallyPlaying, playersBalance, playersName):
    ...
    # Deafult response is no unless specified otherwise
    response = "no"
    ...
    if reallyPlaying:
        while validResponse == False:
            ...
            # Olivia buys everything
            elif playersName == "olivia":
                response = "yes"

            # Amelia buys Red, Yellow, Green, Dark Blue
            elif playersName == "amelia":
                if tile in ["Brown", "Red", "Yellow", "Green", "Dark Blue"]:
                    response = "yes"

            # Grace buys Light Blue, Pink, Red
            elif playersName == "grace":
                if tile in ["Light Blue", "Pink", "Red"]:
                    response = "yes"
            
            # Millie buys Brown, Orange, Yellow, Green, Dark Blue
            elif playersName == "millie":
                if tile in ["Brown", "Orange", "Yellow", "Green", "Dark Blue"]:
                    response = "yes"
                    
            # Sarah buys one from each, except full sets for Brown, Orange and Green
            elif playersName == "sarah":
                # She owns it set to false by deaufult
                ownsOne = False

                # Checks each tile to see if she owns one already
                for t in pd:
                    # If it's the correct colour
                    if t[0] == tile:
                        # If she is the owner
                        if t[2] == "sarah":
                            ownsOne = True

                # If she doesn't own one of that colour
                if ownsOne == False:
                    # THen she buys one
                    response = "yes"

                # If it's Brown, Orange or Green, she buys it
                if tile in ["Brown", "Orange", "Green"]:
                    reponse = "yes"

            elif playersName == "scarlett":
                if tile in ["Orange", "Red", "Yellow"]:
                    response = "yes"

            elif playersName == "theo":
                if tile in ["transport", "Utility", "Red", "Yellow", "Green"]:
                    response = "yes"

            elif playersName == "william":
                # He owns it set to false by deaufult
                ownsOne = False

                # Checks each tile to see if he owns one already
                for t in pd:
                    # If it's the correct colour
                    if t[0] == tile:
                        # If he is the owner
                ...
                # If he doesn't own one of that colour
                if ownsOne == False:
                    # Then he buys one
                    response = "yes"

            elif playersName == "joshua":
                if tile in ["Brown", "Light Blue", "Orange"]:
                    response = "yes"

            elif playersName == "luke":
                if tile in ["Brown", "Light Blue", "Green", "Dark Blue"]:
                    response = "yes"

            elif playersName == "elliot":
                if tile in ["Brown", "Dark Blue"]:
                    response = "yes"
                ...
                print(f"User = {playersName}. This player didn't buy this tile ({tile}), as they shouldn't exist?")
            ...
            for box in input_boxes:
                if found == False:
                    ...
                    if t != None:
                        ...
                        if want != "X" and want != "Y" and want!="name":
                            ...
                            if str(t).lower() in acceptableOutputs:
                                acceptedOutput = t
                                # Delete the button so it's not there anymore :D
                                input_boxes.clear()
                                found = True
...
def getPersonalitiesOpinionsOnBuilding(properties, playersData, player, playersName, tile):
    tileName = properties[tile][0]
    numberOfHousesDesired = 0
    playerNumber = player
    playersBalance = playersData[player][1]
    housesPlaced = properties[tile][3]
    housesLeft = playersData[0][3][0]
    hotelsLeft = playersData[0][3][1]
    costPerHouse = properties[tile][5]
    canAfford = playersBalance // costPerHouse

    # How many the player can afford that the tile could hold
    if canAfford + housesPlaced > 5:
        canAfford = 5 - housesPlaced

    # If the players places a hotel
    if canAfford + housesPlaced == 5:
        # If the bank has a hotel left
        if hotelsLeft >= 1:
            canAfford = canAfford
        # If the bank has no hotels left
        else:
            # Player can place all bar 1 of the houses
            canAfford -= 1
    
    # Checks if the bank has enough houses for the request
    if canAfford > housesLeft:
        canAfford = housesLeft

    # canAfford variable states the max number of houses the user is permitted to build (either can afford or the bank has remaining)

    # Builds all possible
    if playersName == "olivia":
        numberOfHousesDesired = canAfford

    # Builds max bar 600M$
    elif playersName == "amelia":
        # Checks how much money they would have after building
        if (playersBalance - canAfford*costPerHouse) < 600:
            numberOfHousesDesired = (playersBalance-600)//costPerHouse
        else:
            # Builds max possible
            numberOfHousesDesired = canAfford

    # Builds max bar 1000$M, maxes Red then Pink then Light Blue
    elif playersName == "grace":
        goAhead = False
        # Checks if the more expensive properties are done first
        if tileName == "Red":
            goAhead = True
        elif tileName == "Pink":
            # Checks red is fully built first
            fullyBuilt = True
            for p in properties:
                # Checks if each tile is Red and if it's fully built and they own it
                if p[0] == "Red" and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                goAhead = True
        elif tileName == "Light Blue":
            # Checks red is fully built first
            fullyBuilt = True
            for p in properties:
                # Checks if each tile is Red or Pink and if it's fully built and they own it
                if (p[0] == "Red" or p[0] == "Pink") and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                goAhead = True

        # If it's all clear to goAhead
        if goAhead:
            # Checks how much money they would have after building
            if (playersBalance - canAfford*costPerHouse) < 1000:
                numberOfHousesDesired = (playersBalance-1000)//costPerHouse
            else:
                # Builds max possible
                numberOfHousesDesired = canAfford

    # Maxes Brown then Green then Orange then Yellow, never Dark Blue
    elif playersName == "millie":
        if tileName == "Brown":
            numberOfHousesDesired = canAfford

        elif tileName == "Green":
            # Checks if Brown is fully maxed
            fullyBuilt = True
            for p in properties:
                if p[0] == "Brown" and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                numberOfHousesDesired = canAfford

        elif tileName == "Orange":
            # Checks if Brown is fully maxed
            fullyBuilt = True
            for p in properties:
                if (p[0] == "Brown" or p[0] == "Green") and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                numberOfHousesDesired = canAfford

        elif tileName == "Yellow":
            # Checks if Brown is fully maxed
            fullyBuilt = True
            for p in properties:
                if (p[0] == "Brown" or p[0] == "Green" or p[0] == "Orange") and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                numberOfHousesDesired = canAfford
        ...
        if tileName == "Green":
            ...
            fullyBuilt = True
            for p in properties:
                if p[0] == "Green" and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                numberOfHousesDesired = canAfford
            ...
            fullyBuilt = True
            for p in properties:
                if (p[0] == "Green" or p[0] == "Brown") and p[3] != 5 and p[2] == playersName:
                    fullyBuilt = False
            if fullyBuilt:
                numberOfHousesDesired = canAfford
        ...
        # Red1
            ...
            if (properties[16][3] == 5 and properties[16][3] == playersName):
                numberOfHousesDesired = canAfford

        # Yellow3
        elif tile == 29:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName):
                numberOfHousesDesired = canAfford

        # Orange3
        elif tile == 19:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName) and (properties[29][3] == 5 and properties[29][3] == playersName):
                numberOfHousesDesired = canAfford

        # Orange2
        elif tile == 18:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName) and (properties[29][3] == 5 and properties[29][3] == playersName) and (properties[19][3] == 5 and properties[19][3] == playersName):
                numberOfHousesDesired = canAfford

        # Yellow1
        elif tile == 26:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName) and (properties[29][3] == 5 and properties[29][3] == playersName) and (properties[19][3] == 5 and properties[19][3] == playersName) and (properties[18][3] == 5 and properties[18][3] == playersName):
                numberOfHousesDesired = canAfford

        # Red3
        elif tile == 24:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName) and (properties[29][3] == 5 and properties[29][3] == playersName) and (properties[19][3] == 5 and properties[19][3] == playersName) and (properties[18][3] == 5 and properties[18][3] == playersName) and (properties[26][3] == 5 and properties[26][3] == playersName):
                numberOfHousesDesired = canAfford

        # Red 2
        elif tile == 23:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName) and (properties[29][3] == 5 and properties[29][3] == playersName) and (properties[19][3] == 5 and properties[19][3] == playersName) and (properties[18][3] == 5 and properties[18][3] == playersName) and (properties[26][3] == 5 and properties[26][3] == playersName) and (properties[24][3] == 5 and properties[24][3] == playersName):
                numberOfHousesDesired = canAfford

        # Yellow 2
        elif tile == 29:
            if (properties[16][3] == 5 and properties[16][3] == playersName) and (properties[21][3] == 5 and properties[21][3] == playersName) and (properties[29][3] == 5 and properties[29][3] == playersName) and (properties[19][3] == 5 and properties[19][3] == playersName) and (properties[18][3] == 5 and properties[18][3] == playersName) and (properties[26][3] == 5 and properties[26][3] == playersName) and (properties[24][3] == 5 and properties[24][3] == playersName) and (properties[23][3] == 5 and properties[23][3] == playersName):
                numberOfHousesDesired = canAfford

        else:
            numberOfHousesDesired = 0

    # Always keeps double the total rent on board plus 100$
    elif playersName == "theo":
        totalRentOnBoard = 0
        for p in properties:
            # If theo doesn't own it and the bank doesn't own it
            if p[2] != playersName and p[2] != "Bank":
                # The rent is calculated
                nOfHouses = p[3]
                rent = p[4][nOfHouses]
                # Then added to the total
                totalRentOnBoard += rent
        lowerLimit = totalRentOnBoard*2 + 100

        # If the player can't afford it, no houses are bought
        if playersBalance - costPerHouse < lowerLimit:
            numberOfHousesDesired = 0
        elif playersBalance - canAfford*costPerHouse >= lowerLimit:
            numberOfHousesDesired = canAfford
        else:
            canAfford = (playersBalance-lowerLimit)//costPerHouse
            # How many the player can afford that the tile could hold
            if canAfford + housesPlaced > 5:
                canAfford = 5 - housesPlaced
            # Checks the Bank can afford it
            if canAfford > housesLeft:
                canAfford = housesLeft
    ...
    # Only to 3 houses on each tile
    elif playersName == "luke":
        if housesPlaced == 3:
            numberOfHousesDesired = 0
        elif housesPlaced + canAfford >= 3:
            numberOfHousesDesired = 3 - housesPlaced
        else:
            numberOfHousesDesired = canAfford
    ...
    # Always bar 200M$
        ...
        # Checks how much money they would have after building
        if (playersBalance - canAfford*costPerHouse) < 100:
            numberOfHousesDesired = (playersBalance-100)//costPerHouse
        else:
            # Builds max possible
            numberOfHousesDesired = canAfford

    
    # If the number of houses desired is negative, it's simply set to 0 :)
    if numberOfHousesDesired < 0:
        numberOfHousesDesired = 0

    return numberOfHousesDesired


def validateBuildingRequests(properties, player, playersData, allHousesList, playersName):
    ...
    while built == False:
        ...
        # Otherwise the player is a personality
        else:
            numberOfHousesDesired = getPersonalitiesOpinionsOnBuilding(properties, playersData, player, playersName, playersData[player][2])
...
def setNumberOfPlayers(reallyPlaying, legalPlayers):
    ...
    while numberOfPlayersDecided == 0:
        ...
        if reallyPlaying == True:
            try:
                ...
                number = int(getUsersInput([2,4], want="X"))
    ...
    whichPlayers = ["User"]
    # -1 cos the players is already one 
    ...
    for i in range(numberOfPlayersDecided):
        ...
        writeText("Which players would you like to play against?", 800,300,fontSize=50)
        writeText("Available Personalities: "+", ".join(legalPlayers).title(),500,600)
        writeText("Current players: You, "+ ", ".join(whichPlayers[1:]).title(),500,800)
        writeText(f"You need to choose another {numberOfPlayersDecided-i} players.",500,900)
        nameInvalid = True
        while nameInvalid:
            
            # Gets the users input on who they want as an opponent
            name = getUsersInput(legalPlayers,want="name").lower()
...
def doTrade(properties, playersData, playersInvolvedInTrade, listOfPlayersNames):
    ...
    for i in range(len(playerOwnedProperties)):
        ...
        addOrRemoveButton = Button("Keep", (700,200+i*50), (70,20), "AddOrRemove1", textColour=(255,255,255), tileName=name,tileNum=p[8])
    ...
    for i in range(len(computerOwnedProperties)):
        ...
        addOrRemoveButton = Button("Keep", (1100,200+i*50), (70,20), "AddOrRemove2", textColour=(255,255,255), tileName=name,tileNum=p[8])
        buttons.append(addOrRemoveButton)
            ...
            if playersData[player][5] and stillPlaying:
                ...
                if playersData[player][4]:
                    ...
                    if diceRoll == 6:
                        ...
                        playersCoords = displayAllPlayers(["Purple", "Red", "Orange", "Blue"], playerCoords, playersData)
                        ...
                        if playersData[player][1] > 300:
                            ...
                            if reallyPlaying:
                                ...
                                wantToLeave = getUsersOpinion("Would you like to pay $300 to leave jail?", bankBalance, propertiesData, playersData, player=playersList[player-1])
                ...
                # If the player is not in jail
                    ...
                    buttons2, playersCoords,  infoButtons, mortgageButtons = displayMainBoard(allHousesList, properties = propertiesData, playerCoords = playerCoords,playersData=playersData)
                ...
                if reallyPlaying == True:
                    keepPlaying = getUsersOpinion("Keep Playing", playersData[player][1], propertiesData, playersData, player=playersList[player-1])
...
def getUsersOpinion(offer, bankBalance, propertiesData, playersData, player="user"):
    player = player.lower()
    ...
    # Only if Red, Yellow, Green, Dark Blue are all owned
    elif player == "amelia":
        if "jail" in offer.lower():
            allOwned = True
            for p in propertiesData:
                if p[0] in ["Red", "Yellow", "Green", "Dark Blue"] and p[2] != "Bank" and p[2] != player:
                    allOwned = False
   ...
   # Leaves straight away
    ...
    # If a player is within 11 moves from a tile they own
    elif player == "millie":
        if "jail" in offer.lower():
            inRange = False
            ownedTiles = []
            # Finds all player owned properties
            for i in range(len(propertiesData)):
                p = propertiesData[i]
                if p[2] == player:
                    ownedTiles.append(i)
            ...
            for i in range(len(playersData)):
                p = playersData[i][0]
                if p != "Bank" and p != "Free Parking" and p != player:
                    currentTile = playersData[i][2]
                    for tile in ownedTiles:
                        if currentTile + 11 > 40:
                            if (currentTile < tile) or ( (40 - currentTile + 11) < tile):
                                inRange = True
                        elif currentTile < tile and tile <= (currentTile + 11):
                            inRange = True

            # If any player is within 11 moves of a tile they own, then leave jail
            if inRange:
                return "Y"
            else:
                return "N"
            
    # If a player is within 6 moves of a property that would pay off the cost of leaving jail OR it could make a player bankrupt
    elif player == "sarah":
        rent = 0
        if "jail" in offer.lower():
            inRange = False
            ...
            for i in range(len(playersData)):
                ...
                if p != "Bank" and p != "Free Parking" and p != player:
                    ...
                    for tile in ownedTiles:
                        if currentTile + 6 > 40:
                            if (currentTile < tile) or ( (40 - currentTile + 6) < tile):
                                inRange = True
                                numOfHouses = propertiesData[tile][3]
                                newRent = propertiesData[tile][4][numOfHouses]
                                rent += newRent

                                # Check if it could make that player bankrupt
                                if playersData[i][1] - newRent <= 0:
                                    playerCouldGoBankrupt = True
                                
                        elif currentTile < tile and tile <= (currentTile + 6):
                            inRange = True
                            numOfHouses = propertiesData[tile][3]
                            newRent = propertiesData[tile][4][numOfHouses]
                            rent += newRent
            ...
            # If any player is within 6 moves of a tile they own and the total possible rent > cost of leaving jail, then leave jail
            if inRange and rent >= 300:
                return "Y"
            
            # Or if it would result in that player going bankrupt
            elif playerCouldGoBankrupt:
                return "Y"
            
            else:
                return "N"

    # Only if one or more players are between Jail and Go To Jail
    elif player == "scarlett":
        if "jail" in offer.lower():
            leave = False
            for p in playersData:
                if p[0] != "Bank" and p[0] != "Free Parking" and p != player:
                    if p[2] <= 30 and p[2] >= 10:
                        leave = True
    ...
    # Leaves if they have at least 1k and enough to pay rent for the remaining tiles between them and GO
    elif player == "theo":
        if "jail" in offer.lower():
            totalRent = 0
            # Just the tiles after jail are counted
            for t in propertiesData[10:]:
                # If owned by another player
                if t[2] != "Bank" and t[2] != player:
                    numOfHouses = t[3]
                    rent = t[4][numOfHouses]
                    totalRent += rent
            
            # If the player has enough to cover all rent and 
            if totalRent < bankBalance and bankBalance >= 1000:
                return "Y"
            else:
                return "N"
...
legalPlayers = ["olivia", "amelia", "grace", "millie", "sarah", "scarlett", "theo", "william", "joshua", "luke", "elliot"]
